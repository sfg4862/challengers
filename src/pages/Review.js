    ...
    return (
        <div className={styles.reviewContainer}>
            ...
            <div className={styles.temp2}>
                <textarea className={styles.reviewForm}
                    placeholder="리뷰를 작성하세요..."
                    value={newReview}
                    onChange={(e) => setNewReview(e.target.value)}
                />
                <button className={styles.temp} onClick={handleAddReview} style={{}}>작성</button>
            </div>
                ...
                {reviews.map((review) => (
                    <li key={review.id} className={styles.reviewItem}>
                        ...
                        {storedUser === review.userId && (
                            <div>
                                <button className={styles.reviewButton} onClick={() => handleUpdateReview(review.id)}>수정</button>
                                <button className={styles.reviewButton} onClick={() => handleDeleteReview(review.id)}>삭제</button>
                            </div>
                        )}
                    </li>
                ))}
