    ...
    return (
        <>
            <div className={styles.challengeDetail}>
                ...
                <p><strong>진행 상태:</strong> {challenge.status==1 ? "진행중" : "모집중"}</p>
                <p><strong>시작일:</strong> {challenge.startDate} </p>
                <p><strong>현재 단계:</strong> {challenge.totalStep} 단계</p>
                <div className={styles.buttonGroup}>
                    {isAuthor && (
                        <>
                            <button onClick={() => navigate(`/editchallenge/${challenge.id}`)}
                                    className={styles.editButton}>수정
                            </button>
